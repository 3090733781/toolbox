...
<p style="font-size:13px;color:#888;margin-bottom:16px">上传 ZIP 包，包内需要包含 plugin.json 清单文件。</p>
...
function loadPluginCenter(){var el=document.getElementById('pluginCenterList');el.innerHTML='<div class="loading"><div class="spinner"></div>正在读取插件中心...</div>';fetch('../api/index.php?source=plugin_center_list').then(r=>r.json()).then(function(d){if(!d.success){el.innerHTML='<div class=err>'+esc(d.error||'读取失败')+'</div>';return}if(!d.data.length){el.innerHTML=renderPluginAuth(d.authorization)+'<div style="text-align:center;padding:28px;color:#aaa">插件中心暂无插件</div>';return}el.innerHTML=renderPluginCenter(d.data,d.authorization);bindPluginButtons()}).catch(function(){el.innerHTML='<div class=err>读取插件中心失败</div>'})}
function installRemotePlugin(id,name){if(!confirm('确认安装插件「'+name+'」？'))return;var el=document.getElementById('pluginCenterList');el.innerHTML='<div class="loading"><div class="spinner"></div>正在下载并安装插件...</div>';fetch('../api/index.php?source=plugin_center_install',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:id})}).then(r=>r.json()).then(function(d){if(d.success){el.innerHTML='<div class=msg style=display:block>插件「'+esc((d.data.manifest&&d.data.manifest.name)||d.data.name)+'」安装成功，正在跳转...</div>';setTimeout(function(){window.location.href='?p=plugins'},1200)}else{el.innerHTML='<div class=err>'+esc(d.error||'安装失败')+'</div><div style="margin-top:12px"><button class="btn btn-sm" onclick="loadPluginCenter()">重新加载</button></div>'}}).catch(function(){el.innerHTML='<div class=err>网络错误</div>'})}
function installLocalPlugin(i){var f=i.files[0];if(!f)return;var el=document.getElementById('pluginInstallResult');el.innerHTML='<div class=loading><div class=spinner></div>安装中...</div>';var fd=new FormData();fd.append('file',f);fetch('../api/index.php?source=plugin_install',{method:'POST',body:fd}).then(r=>r.json()).then(function(d){if(d.success){el.innerHTML='<div class=msg style=display:block>插件「'+esc((d.data.manifest&&d.data.manifest.name)||d.data.name)+'」安装成功，正在跳转...</div>';setTimeout(function(){window.location.href='?p=plugins'},1200)}else el.innerHTML='<div class=err>'+esc(d.error||'安装失败')+'</div>'}).catch(function(){el.innerHTML='<div class=err>网络错误</div>'})}
loadPluginCenter();
</script>
