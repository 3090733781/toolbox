...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $step === 3) {
    ...
    try {
        ...
        if (!$err) {
            ...
            $pdo->exec("CREATE TABLE IF NOT EXISTS `api_keys` (`id` INT AUTO_INCREMENT PRIMARY KEY, `user_id` INT NOT NULL, `name` VARCHAR(100) NOT NULL DEFAULT 'Default', `api_key` VARCHAR(64) NOT NULL UNIQUE, `permissions` VARCHAR(500) DEFAULT '*', `rate_limit` INT DEFAULT 60, `status` TINYINT DEFAULT 1, `expires_at` DATETIME DEFAULT NULL, `used_count` BIGINT DEFAULT 0, `last_used_at` DATETIME DEFAULT NULL, `last_used_ip` VARCHAR(45) DEFAULT NULL, `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP, INDEX idx_user (`user_id`), INDEX idx_key (`api_key`), INDEX idx_status (`status`)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
            $pdo->exec("CREATE TABLE IF NOT EXISTS `api_logs` (`id` BIGINT AUTO_INCREMENT PRIMARY KEY, `api_key_id` INT DEFAULT NULL, `user_id` INT DEFAULT NULL, `source` VARCHAR(50) NOT NULL, `ip` VARCHAR(45) DEFAULT NULL, `user_agent` VARCHAR(255) DEFAULT NULL, `status` VARCHAR(10) DEFAULT 'ok', `error` VARCHAR(500) DEFAULT NULL, `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP, INDEX idx_key (`api_key_id`), INDEX idx_source (`source`), INDEX idx_time (`created_at`), INDEX idx_user (`user_id`)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
            $stmt = $pdo->prepare("INSERT IGNORE INTO `settings` (`key`, `value`) VALUES ('db_version', '2.0.0')");
            $stmt->execute();
